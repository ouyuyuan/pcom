...
! Last Change: 2016-05-12 19:37:35 BJT
...
subroutine arrays_allocate () !{{{1
  ...
  call arrays_init( acts%var, ni,nj,nk, 0.0, g12 )
  acts%n = 0; acts%nrec = 0

  call arrays_init( acuv%var, ni,nj,nk, 0.0, g32 )
  acuv%n = 0; acuv%nrec = 0

  ! acw lies on g32 just for output, this is ad hoc
  call arrays_init( acw%var, ni,nj,nk, 0.0, g32 )
  acw%n = 0; acw%nrec = 0

  call arrays_init( acssh%var, ni,nj, 0.0, g1j )
  acssh%n = 0; acssh%nrec = 0

  call arrays_init(ts(tc), ni,nj,nk, 0.0, g12)
  call arrays_init(ts(tp), ni,nj,nk, 0.0, g12)

  call arrays_init(adv(tc), ni,nj,nk, 0.0, g32)
  call arrays_init(adv(tp), ni,nj,nk, 0.0, g32)
  call arrays_init(adv(tpp), ni,nj,nk, 0.0, g32)

  call arrays_init(up(tc), ni,nj,nk, 0.0, g32)
  call arrays_init(up(tp), ni,nj,nk, 0.0, g32)

  call arrays_init(fri, ni,nj,nk, 0.0, g32)
  ...
  call arrays_init(bnd%tau, ni,nj, 0.0, g1j)
  call arrays_init(bnd%ts,  ni,nj, 0.0, g1j)
  call arrays_init(bnd%pa,  ni,nj, 0.0, g1j)
  call arrays_init(bnd%fw,  ni,nj, 0.0, g1j)

  ! initiate as fresh water
  call arrays_init(alpha, ni,nj,nk, 1/1000.0, g12) 
  call arrays_init(adp, ni,nj,nk, 0.0, g12) 

  call arrays_init(wm, ni,nj,nkp, 0.0, g11)

  call arrays_init(am, ni,nj,nk,  am_c, g32)
  call arrays_init(km, ni,nj,nkp, km_c, g32%ud)
  call arrays_init(kh, ni,nj,nkp, 0.0,  g12%ud)

  call arrays_init(graphih, ni,nj, 0.0, g3j)

  call arrays_init(grapa, ni,nj, 0.0, g3j)

  ch%hg => g1j
  call arrays_init(ch%tc, ni,nj, 1.0)
  call arrays_init(ch%tp, ni,nj, 1.0)
  call arrays_init(ch%bc, ni,nj, 1.0)
  call arrays_init(ch%bc2, ni,nj, 1.0)

  call arrays_init(cor, ni,nj, 0.0, g3j)

  call arrays_init(upb(tc), ni,nj, 0.0, g3j)
  call arrays_init(upb(tp), ni,nj, 0.0, g3j)

  call arrays_init(badv, ni,nj, 0.0, g3j)
